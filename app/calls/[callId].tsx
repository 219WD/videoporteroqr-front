...
export default function CallScreen() {
  ...
  const anonymousModeRef = useRef(false);
  ...
  }, [anonymousMode, forcedRole, session, user?.id]);
  ...
  const sendEndCall = useCallback(async () => {
    ...
    try {
      setEnding(true);
      if (anonymousMode) {
        await api.post('/videocall/end-call', { callId });
      } else {
  ...
  }, [anonymousMode, callId, ending, leaveScreen]);
  ...
  const ensurePeerConnection = useCallback(() => {
    ...
    const peer = createCallPeerConnection({
      ...
      onIceCandidate: (event) => {
        if (!event?.candidate || !socketRef.current || !callId || !user?.id) {
          return;
        }
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      let anonymousGuestName = 'Visitante';
      ...
      try {
        ...
        try {
          ...
          loaded = normalizeCall(response.data.call);
          setSession(loaded);
          remoteUserIdRef.current = isCaller ? loaded?.callee?.id?.toString?.() || loaded?.callee?.id : loaded?.caller?.id?.toString?.() || loaded?.caller?.id;
          setStatus(isCaller ? 'Llamando...' : 'Te están llamando');
          setWaitingAcceptance(false);
          setAccepted(loaded?.status === 'accepted');
        } catch (sessionError: any) {
          const notFound = sessionError?.response?.status === 404;
          if (!notFound) {
            throw sessionError;
          }

          const flowResponse = await api.get(`/flows/status/${callId}`);
          if (!mounted) return;

          const flow = flowResponse.data?.call || null;
          anonymousCall = true;
          anonymousGuestName = flow?.guestName || 'Visitante';
          setAnonymousMode(true);
          anonymousModeRef.current = true;
          setSession({
            callId,
            status: flow?.status || 'pending',
            response: flow?.response || null,
            actionType: flow?.actionType || 'call',
            caller: {
              id: user.id,
              name: user.name,
              email: null,
            ...
            callee: {
              id: null,
              name: anonymousGuestName,
              email: null,
              role: 'guest',
            },
          });
          remoteUserIdRef.current = null;
          setStatus(flow?.response === 'accept' ? 'Conectando...' : 'Te están llamando desde el chat');
          setWaitingAcceptance(false);
          setAccepted(flow?.response === 'accept' || flow?.status === 'answered');
        }
        ...
        if (anonymousCall) {
          ...
          socket.on('call:offer', async (payload) => {
            ...
            if (!localStreamRef.current) {
              ...
              setStatus('Preparando cámara para responder...');
              return;
            }
          ...
          socket.on('call:ice-candidate', async (payload) => {
            ...
            try {
              ...
              console.warn('[call] error agregando ICE candidate anonimo:', error);
            }
          });
            ...
            .then((stream) => {
              ...
              if (pendingOfferRef.current) {
                ...
                peer.setRemoteDescription(new RTCSessionDescription(offer))
                  ...
                  .catch((error) => {
                    console.error('[call] error procesando offer pendiente anonimo:', error);
                  });
              }
            })
            .catch((streamError) => {
              console.error('[call] error obteniendo stream local anonimo:', streamError);
              if (mounted) {
                setStatus('Necesitamos acceso a la cámara y al micrófono');
              }
            });
        ...
        socket.on('call:rejected', (payload) => {
          if (payload?.callId !== callId) return;
          Alert.alert('Llamada rechazada', 'La otra persona rechazó la llamada');
          leaveScreen();
        });
        ...
        const handleCallTimeout = (payload) => {
          ...
          const title = status === 'timeout' ? 'Llamada no contestada' : 'Llamada finalizada';
  ...
  const remoteLabel = anonymousMode
    ? session?.callee?.name || 'Visitante'
    : (isCaller ? session?.callee?.name || 'Contacto' : session?.caller?.name || 'Contacto');
      ...
      {remoteStreamUrl ? (
        ...
        <View style={styles.placeholder}>
          ...
          <Text style={styles.placeholderText}>
            {isCaller ? 'Esperando respuesta del contacto...' : 'Esperando conexión...'}
          </Text>
        ...
        {isCallee && !accepted ? (
          ...
          <>
            ...
            <TouchableOpacity style={[styles.controlButton, !videoEnabled && styles.controlButtonActive]} onPress={toggleVideo}>
              ...
              <Text style={styles.controlLabel}>{videoEnabled ? 'Video' : 'Video apagado'}</Text>
            </TouchableOpacity>
